...
// Use the same Post interface as in social types
...
export const usePosts = (options: UsePostsOptions = {}): UsePostsReturn => {
  ...
  const fetchPosts = useCallback(async (options: FetchPostsOptions = {}) => {
    ...
    while (retries <= maxRetries) {
      try {
        console.log(`Fetching posts - Page: ${pageNum}, Feed: ${currentFeedType}${retries > 0 ? ` (retry ${retries})` : ''}`);
        ...
        // Use the API service instead of direct fetch for proper timeout handling
        const response: any = await api.posts.getAll(queryParams);
  ...
  const fetchBookmarkedPosts = useCallback(async (userId: string) => {
    ...
    while (retries <= maxRetries) {
      try {
        ...
        // Use the API service instead of direct fetch for proper timeout handling
        const response: any = await api.posts.getBookmarkedPosts(userId, { limit, page: 1 });
  ...
  const sharePost = useCallback(async (postId: string) => {
    ...
    try {
      // Make API call to share the post
      const response: any = await api.posts.share(postId, 'internal');
      ...
      if (response && response.success) {
        ...
        setPosts(prev => prev.map(post => 
          post.id === postId ? { 
            ...post, 
            shareCount: response.data?.shareCount || (post.shareCount || 0) + 1,
            isShared: true
          } : post
        ));
